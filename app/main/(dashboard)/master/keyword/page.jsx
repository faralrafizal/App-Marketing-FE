...
export default function Page() {
  ...
  const Role = dataUser?.role_access["Master Keyword"]?.map((res) => res.event)
  ...
  const addMaster = async (data) => {
    Swal.fire(
      {
        title: data?.id_keyword ? "Edit Data" : "Masukan Data",
        input: "text",
        inputValue: data?.keyword,
        focusConfirm: false,
        reverseButtons: true,
        showCancelButton: true,
        confirmButtonText: data?.id_keyword ? "Save Edit" : "Save",
        ...
        preConfirm: async (val) => {
          ...
          if (data?.id_keyword) {
            body.id = data?.id_keyword
          }
          await FetchData({
            methode: data?.id_keyword ? "put" : "post",
            url: data?.id_keyword ? "/edit-keyword" : "/add-keyword",
            data: body,
            token: token
          }).then((data) => {
            console.log(data);
  ...
  let deleteMaster = async () => {
    await FetchData({
      methode: "delete",
      url: `/delete-keyword?id=${idDelete}`,
      token: token
    });
    setRefreshPage(!refreshPage);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            {Role?.includes('Create') && <div
              onClick={addMaster}
              className="bg-[#5D5FEF] px-4 cursor-pointer py-2 rounded-full flex items-center gap-2"
            >
              <Image
                className=""
